...
import { getEventRegistrations } from "@/utils/db/group-registrations.ts";
...
/**
 * Admin event registrations detail page — Fresh v2
 */
...
export const handler = define.handlers<EventRegistrationsData>({
  async GET(ctx) {
    ...
    const event = eventId
      ? await getEventById(eventId, ctx.state)
      : await getNextAvailableEvent(eventSlug, ctx.state);

    if (!event) {
      return new Response("Event not found", { status: 404 });
    }

    const registrations = await getEventRegistrations(event.id, ctx.state);
