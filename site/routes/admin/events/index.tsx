...
import { getAllEvents } from "@/utils/db/group-events.ts";
import type { EventConfig } from "@/utils/db/group-events.ts";
import { getActiveRegistrationCount } from "@/utils/db/group-registrations.ts";
import ReminderTrigger from "@/islands/ReminderTrigger.tsx";

/**
 * Admin events list — Fresh v2
 */
...
export const handler = define.handlers<AdminEventsData>({
  async GET(ctx) {
    const events = await getAllEvents(ctx.state);
    const eventsWithStats = await Promise.all(
      events.map(async (event) => ({
        ...event,
        registrationCount: await getActiveRegistrationCount(
          event.id,
          ctx.state,
        ),
      })),
    );
    ctx.state.adminBreadcrumbs = [{ label: "Events" }];
    return page({ events: eventsWithStats });
  },
});

export default define.page<typeof handler>(function AdminEventsPage({ data }) {
  ...
  return (
    <div class="max-w-7xl mx-auto px-6 py-8">
      {/* Header */}
      <h1 class="text-2xl font-bold text-gray-900 mb-8">Event Management</h1>
        ...
        <div class="mb-8">
          ...
          <div class="space-y-4">
            {upcomingEvents.map((event) => {
              ...
              const percentFull = (event.registrationCount / event.capacity) *
                100;
              return (
                <div
                  key={event.id}
