...
import { cancelRegistration, getEventById } from '../../../../../utils/events.ts';
	...
	async POST( ctx) {
		try {
			...
			if (!eventId || !registrationId) {
				console.error('APIEventCancel - Must supply both eventId and registrationId', { eventId, registrationId });
				return new Response(
			...
			if (!result.success) {
				const status = result.error === 'Registration not found' ? 404 :
					result.error === 'Registration already cancelled' ? 400 : 500;
				return new Response(
