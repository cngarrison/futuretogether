...
export default define.page(async function SpeakerPage({ params }) {
  ...
  if (!meta) {
    return <div style="padding:2rem;color:white;background:#0d1117;min-height:100vh;">Slideshow not found.</div>;
  }
  ...
  return (
    <>
      <Head><title>Speaker Notes \u2014 {meta.title}</title></Head>
      ...
      <div style="background:#0d1117;min-height:100dvh;padding:1rem;overflow-y:auto;">
        <div style="max-width:480px;margin:0 auto;padding-bottom:4rem;">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem;padding-bottom:0.75rem;border-bottom:1px solid rgba(255,255,255,0.1);">
            <span style="font-size:0.75rem;color:rgba(255,255,255,0.5);">{meta.title}</span>
            <a href={`/slideshows/${slug}/controller`} style="font-size:0.7rem;color:#c4853a;">Controller \u2192</a>
          </div>
          {slides.map((slide: SlideData, i: number) => (
            <SpeakerCard key={slide.id} slide={slide} index={i} total={slides.length} />
          ))}
        </div>
      </div>
      <SpeakerNotesSync role="receiver" slideCount={slides.length} />
      <ConnectionStatus />
    </>
  );
});
