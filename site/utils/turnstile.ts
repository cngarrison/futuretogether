...
export async function verifyTurnstileToken(token: string): Promise<boolean> {
	...
	try {
		...
		if (!response.ok) {
			console.error('Turnstile: Verification API returned error:', response.status);
			return false;
		}
		...
		if (data.success) {
			...
			console.warn('Turnstile: Token verification failed:', data['error-codes'] || 'unknown error');
			return false;
		}
	} catch (error) {
