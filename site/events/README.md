...
This directory contains YAML configuration files for events. Each file defines a separate event instance.
...
The public page `/events/{slug}` will automatically show the next available event based on:
- `isActive: true`
...
3. **Deadline Passed**: Within registration deadline hours (switches to next event)
