...
export default function SlideController(
  { slides, totalDurationMinutes = 45 }: Props,
) {
  ...
  return (
    <div style="background:#0f1923;min-height:100dvh;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:2rem;color:white;font-family:system-ui,sans-serif;">
      {/* Timer */}
      <div
        style={`font-size:3.5rem;font-weight:700;font-variant-numeric:tabular-nums;color:${timerColour};letter-spacing:-0.02em;margin-bottom:0.25rem;`}
      >
        {overTime ? "-" : ""}
        {fmtTime(mins, secs)}
      </div>
      ...
      <div style="text-align:center;margin-bottom:2rem;max-width:320px;">
        <div style="font-size:0.65rem;opacity:0.4;text-transform:uppercase;letter-spacing:0.1em;margin-bottom:0.5rem;">
          Slide {index + 1} / {slides.length}
        </div>
        <div style="font-size:1.1rem;font-weight:600;line-height:1.35;">
          {slide?.title ?? "…"}
        </div>
        {slide?.notes?.anchor && (
          <div style="font-size:0.8rem;color:#7dd3fc;margin-top:0.4rem;opacity:0.8;">
            {slide.notes.anchor}
          </div>
        )}
        {slide?.notes && (
          <div style="font-size:0.7rem;opacity:0.35;margin-top:0.35rem;">
            {slide.notes.bullets.length}{" "}
            note{slide.notes.bullets.length !== 1 ? "s" : ""}
