...
export default function EventDateTime(
  {
    ...
    timezone,
    showIcon = true,
    iconClass = "w-5 h-5 mr-1.5 opacity-80",
  ...
  if (!mounted || !userTimezone) {
    ...
    return (
      <div class={`flex items-start ${textClass}`}>
        ...
        <div>
          <div>
            {eventFormatted.date} • {eventFormatted.time}{" "}
            {eventFormatted.timezoneAbbr}
          </div>
        </div>
      </div>
    );
  ...
  const showBothTimezones = !areTimezonesEquivalent(
    eventFormatted,
    userFormatted,
  );
  ...
  if (!showBothTimezones) {
    ...
    return (
      <div class={`flex items-start ${textClass}`}>
        ...
        <div>
          <div>
            {userFormatted.date} • {userFormatted.time}{" "}
            {userFormatted.timezoneAbbr}
          </div>
        </div>
      </div>
    );
    ...
    <div class={`flex items-start ${textClass}`}>
      ...
      <div>
        ...
        <div class="text-sm text-gray-600 mt-1">
          Event time: {eventFormatted.date} • {eventFormatted.time}{" "}
          {eventFormatted.timezoneAbbr}
        </div>
      </div>
    </div>
