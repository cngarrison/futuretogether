# Event Plan — Future Together: Tumbarumba Community Event
**Working document** · Prepared for Laura [venue owner] and Tumbarumba Chamber of Commerce

---

## Event Overview

| | |
|---|---|
| **Title** | The Future is Arriving. Is Tumbarumba Ready? |
| **Subtitle** | A free community talk on AI, work, and what's coming next for our region |
| **Presenter** | Charlie Garrison, Future Together |
| **Supported by** | Tumbarumba Chamber of Commerce |
| **Venue** | The Nest Café Theatre, Tumbarumba |
| **Date** | ⚠️ TO CONFIRM WITH LAURA |
| **Doors open** | 6:15 pm |
| **Talk starts** | 6:30 pm |
| **Estimated finish** | 7:45–8:00 pm |
| **Entry** | Free |
| **Capacity** | TBD — confirm with Laura (theatre seating) |
| **Registration** | Encouraged but not required — via futuretogether.community |

---

## Our Purpose

### Why We're Doing This

Something significant is happening, and most people in Tumbarumba — like most people everywhere — don't yet have a clear picture of it. The technology reshaping how we work, run businesses, and organise our lives is moving faster than public awareness. That gap between what's actually happening and what most people understand is the problem this event exists to close.

This isn't an abstract concern. Rural and regional communities face particular exposure — fewer alternative employers, longer distances to services, tighter margins in business. But they also hold real strengths: existing community bonds, local institutions that still function, and people who know how to look out for each other.

Tumbarumba has something else too. After a decade of campaigning, this community has just won back its local council. That outcome — achieved through sustained, organised civic effort — is a reminder of what's possible when a community decides to act together. A new council starting fresh is also a genuine opportunity: to shape local priorities, ask new questions, and ensure the community's voice is heard in decisions that will define the next decade.

Future Together, the Tumbarumba Chamber of Commerce, and The Nest café are hosting this event because this community deserves the chance to face what's coming together — informed, connected, and with a sense of agency rather than anxiety. Not panic. Not paralysis. A conversation that leads to action.

---

### What We Want to Achieve

**On the night**
Thirty people leave better informed and less alone with their concerns. The questions raised during Q&A are ones people have been carrying privately — and the room discovers they're not alone in asking them. The conversation continues at the café bar after the formal close.

**In the weeks that follow**
A cohort of engaged locals who want to keep meeting — the foundation for an ongoing community group in Tumbarumba. Chamber members and community leaders see the value in continuing to convene these conversations. The event provides a template that other local organisations (Rotary, schools, sporting clubs) can build on.

**For the new council**
Local councillors — incoming and outgoing — are aware that their community is thinking about these issues. Future Together provides a non-partisan, civic-minded space for that conversation to mature into policy interest and local action.

**For the region**
Nearby communities — Batlow, Rosewood, Tumut, Walwa, Corryong, and Khancoban — hear what Tumbarumba did and want their own conversation. The Snowy Valleys region develops a reputation as a place that takes its future seriously.

**For rural Australia**
The Tumbarumba event becomes a model that any community can follow — proof that a small regional town can engage seriously with one of the defining issues of our time, without waiting for someone else to start the conversation.

---

### What Success Looks Like

- The room is full, or close to it
- At least 10 people sign up to stay connected or attend a future event
- At least one person says: *"I had no idea — I'm glad someone is talking about this"*
- Chamber members and community leaders want to co-host the next event
- A follow-up event is planned within three months — and this time, it fills a bigger room

---

## Run Sheet

| Time | What's happening |
|---|---|
| **6:15 pm** | Doors open. Café bar available. Guests arriving and settling. |
| **6:30 pm** | Welcome by Chamber of Commerce representative (2–3 min) |
| **6:33 pm** | Introduction of Charlie Garrison by Laura or Chamber rep (1–2 min) |
| **6:35 pm** | Talk begins — Charlie presents (approx. 45 min) |
| **7:20 pm** | Q&A — moderated by Charlie (approx. 20 min) |
| **7:40 pm** | Closing remarks — Charlie (2–3 min): what's next, how to stay connected |
| **7:43 pm** | Informal conversation continues — café bar available |
| **8:00 pm** | Venue close (or as agreed with Laura) |

---

## Talk Structure

The presentation uses a slideshow (displayed on screen/projector) adapted from the existing Future Together meetup format. Estimated 15 slides, 45 minutes.

This is **a public talk with Q&A**, not an intimate group discussion — the slides and delivery have been adapted accordingly.

### Content Arc

| # | Slide | Purpose | Approx. time |
|---|---|---|---|
| 1 | **Opening** — Future Together intro, event framing | Set tone, welcome audience | 2 min |
| 2 | **The Timeline Has Collapsed** — "What we thought was 10 years away is arriving in 1–3 years" | The core urgency hook | 3 min |
| 3 | **We Must Solve Alignment First** — AI deception/manipulation in controlled tests | The safety problem, grounded in evidence | 3 min |
| 4 | **The Singularity: A One-Way Door** — recursive self-improvement | Explains why we can't "undo" this | 3 min |
| 5 | **We Can't Slow Down** — race dynamics | Why pausing isn't an option | 2 min |
| 6 | **What I'm Hearing** — job impacts across sectors, robotics | Brings it to human scale | 3 min |
| 7 | **50/50: Utopia or Dystopia** — split view | Honest framing: two real paths | 3 min |
| 8 | **Remember February 2020?** — the analogy | Lands the "we're in that phase now" point | 3 min |
| 9 | **Immediate Human Risks** — jobs, food, purpose, welfare gap | What this means for ordinary life | 4 min |
| 10 | **What Does a Good Future Look Like?** — personal + societal | Balances the risk framing with genuine hope | 3 min |
| 11 | **Why Community Is the Answer** — Awareness → Conversation → Action | Why we're all in this room | 3 min |
| 12 | **Back to Basics** — Maslow's hierarchy | Grounding: what do we need to protect first? | 3 min |
| 13 | **What Can We Do?** — 6 practical actions | Ends on action, not anxiety | 4 min |
| 14 | **The Bigger Picture** — AI + climate + geopolitics + pandemic | Context: AI isn't the only disruption | 2 min |
| 15 | **Resources & Close** — QR codes, futuretogether.community, what's next | CTA: join, register, share | 2 min |

### Adaptations from the Conversation Format

The source slideshow is designed for an intimate group conversation. The following changes will be made for this public event:

- **Opening slide:** Remove "This is a discussion, not a presentation" framing. Replace with a brief welcome and event overview.
- **Discussion questions:** Transform from open group prompts to rhetorical questions that invite reflection. Audience responds during Q&A, not mid-slide.
- **Slide 6 (What I'm Hearing):** Add a regional angle — what this means for rural communities and small towns like Tumbarumba specifically.
- **Slide 15 (Closing):** Replace Slack invite with an in-person CTA: sign up to stay informed, register for the next event, bring a friend next time.
- **Maslow image:** Slide 12 references `maslow.png` — this will need to be available locally or online when presenting.

---

## Technical & AV Requirements

*Items marked ⚠️ need confirmation from Laura.*

| Requirement | Detail | Status |
|---|---|---|
| **Screen / sound** | The theatre is an existing movie theatre — a screen and sound system are already in place. | ⚠️ Confirm screen size, projection setup, and available laptop connections |
| **Laptop connection** | HDMI is preferred. | ⚠️ Confirm connector type and whether an adapter is needed |
| **Microphone** | For a theatre audience, a handheld or lapel mic is strongly recommended. | ⚠️ Confirm available or arrange to borrow one |
| **Internet connection** | Slideshow loads fonts from Google Fonts. WiFi preferred; phone hotspot available as fallback. | ⚠️ Confirm venue WiFi if available |
| **Power point** | For presenter's laptop at the front. | ⚠️ Confirm location |
| **Lighting** | Room should be dim enough to see the screen clearly, bright enough for audience to feel comfortable. | ⚠️ Discuss with Laura |
| **Presenter's laptop** | Charlie will bring. Mac — will need appropriate adapter if venue uses HDMI only. | Charlie to bring |

---

## Room Setup

| Element | Detail |
|---|---|
| **Seating** | Existing theatre seating (rows facing screen). ⚠️ Confirm seating capacity with Laura — room may be large enough but chairs may need supplementing. |
| **Theatre access** | Via inside the café — large room at the rear. Guests enter through the café. |
| **Presenter position** | Front of room, to the side of the screen so it remains visible |
| **Welcome table** | Small table at entry for optional registration sign-in, printed resources, event leaflets |
| **Signage** | Future Together + Tumbarumba Chamber of Commerce logos at entry and/or on screen before the talk starts |
| **Café bar** | Available from 6:15 pm (before) and from 7:43 pm (after) — guests encouraged to stay and talk |

---

## Registration & Door

- Guests encouraged to register in advance at **futuretogether.community** (not required)
- Registration gives us a headcount and a mailing list for future events
- At the door: simple sign-in sheet (name + email, optional) for walk-ins
- No ticket, no ID required — just show up
- If capacity is reached, a sign on the door: "Full tonight — scan for next event"

---

## Promotion Plan

| Channel | Detail | Who |
|---|---|---|
| **Local newspaper ad** | Quarter-page or classified — drafted and ready to submit | Charlie / Chamber |
| **Chamber of Commerce** | Member newsletter, member WhatsApp/email groups | Chamber |
| **The Nest café** | In-café signage, social media, word of mouth from Laura | Laura |
| **Rotary Club** | Approach once Chamber confirmed — present as civic education event | Charlie |
| **Other local groups** | Schools (P&C), RSL, sporting clubs, churches — newsletters and noticeboards | Charlie |
| **futuretogether.community** | Event listed on website with registration link | Charlie |
| **Social media** | Future Together social posts | Charlie |

---

## Questions for Laura

The following need Laura's input before we can finalise the plan:

1. **Date** — what Thursday evenings is the theatre available in June or early July?
2. **AV** — what screen and projection setup is in the theatre? What laptop connections are available (HDMI? other)?
3. **Microphone** — is there a mic available, or can one be borrowed?
4. **Seating** — what is the current seating capacity? Would additional chairs be possible if needed?
5. **Café bar** — is it feasible to have the bar open from 6:15 pm (before) and again from around 7:45 pm (after)?
6. **Timing** — is 6:15 pm doors / 6:30 pm start / 8:00 pm finish workable for the venue?
7. **Anything else** — what would Laura want to know or flag that we haven't covered?

---

## Next Steps

| Step | Owner | When |
|---|---|---|
| Laura reviews this plan and provides feedback | Laura | ASAP |
| Confirm date (June or early July) | Laura + Charlie | After Laura's feedback |
| Submit newspaper ad | Charlie + Chamber | Early next week (for next issue) |
| Finalise AV setup plan | Charlie + Laura | Once date confirmed |
| Approach Rotary | Charlie | Once date confirmed |
| Adapt slideshow for public talk format | Charlie + BB | Before event |
| Create event listing on futuretogether.community | Charlie + BB | Once date confirmed |
| Door sign-in sheet | Charlie | Week of event |
| Printed resource list (optional) | Charlie | Week of event |

---

*Future Together · futuretogether.community · Prepared with BB (Beyond Better)*
